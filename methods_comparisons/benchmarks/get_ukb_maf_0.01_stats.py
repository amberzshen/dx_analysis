...
def get_ukb_linarg_size(chroms):
    size_in_memory = []
    for chrom in chroms:        
        linarg_dir = f'/mnt/project/linear_args/ukb20279_maf_0.01/chr{chrom}'
...
def get_vcf_disk_size(chroms):
    disk_size = 0
    for chrom in chroms:
        disk_size += os.path.getsize(f"/mnt/project/Bulk/Previous WGS releases/GATK and GraphTyper WGS/SHAPEIT Phased VCFs/ukb20279_maf_0.01_c{chrom}_b0_v1.vcf.gz")
    return disk_size
...
def get_nnz_ratio(chroms):
    ...
    genotypes_nnz = 0
    n_variants = 0
    linarg_path = '/mnt/project/linear_args/ukb20279_maf_0.01'
    for chrom in chroms:
...
if __name__ == "__main__":
    ...
    with open("ukb20279_maf_0.01_linarg_stats_chrX.txt", "w") as f:
