...
for col in ['p50_i0'] + [f'p22009_a{i}' for i in range(1,41)]:
...
phenotypes_filt.select(["FID", "IID"] + phenotype).write_csv("phenotypes.txt", separator=" ", null_value='0')

# Save covariate file (FID, IID, covariates...)
phenotypes_filt.select(["FID", "IID"] + covariates).write_csv("covariates.txt", separator=" ", null_value='0')
