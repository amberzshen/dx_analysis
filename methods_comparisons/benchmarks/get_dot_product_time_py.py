...
def time_grg_dot_product(grg_path):
    ...
    dp_time, dp_mem = benchmark(pygrgl.dot_product, grg, y, pygrgl.TraversalDirection.UP, repeat=10)
    return load_time, load_mem, dp_time, dp_mem


def time_linarg_dot_product(linarg_path):
    load_time, load_mem = benchmark(LinearARG.read, linarg_path, repeat=10)
    linarg = LinearARG.read(linarg_path, load_metadata=False)
    y = np.ones(linarg.shape[0])
    dp_time, dp_mem = benchmark(lambda: y @ linarg, repeat=10)
    return load_time, load_mem, dp_time, dp_mem


def time_scipy_dot_product(genotypes_path):
    load_time, load_mem = benchmark(load_npz, genotypes_path, repeat=10)
    genotypes = load_npz(genotypes_path)
    y = np.ones(genotypes.shape[0])
    dp_time, dp_mem = benchmark(lambda: y @ genotypes, repeat=10)
    return load_time, load_mem, dp_time, dp_mem
...
if __name__ == "__main__":
    ...
    ## temporary! need to localize file first! ##
    ...
    for chrom in [1, 11, 21]:
        ...
        for partition in os.listdir(linarg_dir):
            ...
            load_time, load_mem, dp_time, dp_mem = time_linarg_dot_product(linarg_path)
            results.append((chrom, f'linarg_{partition}', load_time, load_mem, dp_time, dp_mem))
        ...
        for large_partition in os.listdir(linarg_dir):
            for file in os.listdir(f'{linarg_dir}/{large_partition}/genotype_matrices/'):
                ...
                load_time, load_mem, dp_time, dp_mem = time_scipy_dot_product(genotypes_path)
                results.append((chrom, f'genotypes_{file}', load_time, load_mem, dp_time, dp_mem))
    ...
    df.write_csv("amber/methods_comparisons/results/tmp_dot_product_time_all.csv")