...
def partition_chromosome_X(par1, par2, large_partition_size, small_partition_size, data_identifier, out):
    ...
    chrom = 'chrX'
    par1_start = int(par1.sep('-')[1])
    par1_end = int(par1.sep('-')[2])
    par2_start = int(par2.sep('-')[1])
    par2_end = int(par2.sep('-')[2])
    ...
    for i in range(1, len(large_partitions)+1):
    ...
    with open(f'{large_partition_dir}/partitions.txt', 'w') as f:
        f.write(' '.join(['chrom', 'partition_number', 'start', 'end'])+'\n')
        f.write(' '.join([chrom, 0, par1_start, par1_end])+'\n')
        
    large_partition_dir = f'{out}/{data_identifier}/{chrom}/{len(large_partitions)+1}_{chrom}-{par1_start}-{par1_end}'
    if not os.path.exists(large_partition_dir): os.makedirs(large_partition_dir)
    with open(f'{large_partition_dir}/partitions.txt', 'w') as f:
        f.write(' '.join(['chrom', 'partition_number', 'start', 'end'])+'\n')
        f.write(' '.join([chrom, 0, par2_start, par2_end])+'\n')
