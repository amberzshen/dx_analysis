...
if __name__ == "__main__":
    ...
    results = None
    for f in files:
        linarg = ld.LinearARG.read(f'{linarg_dir}/{f}/linear_arg_individual.h5')
        linarg.iids = pl.Series("whitelist", np.repeat(whitelist, 2)).cast(pl.Int64)
        res = run_gwas(linarg, phenotypes_filt.lazy(), phenotype, covariates, assume_hwe=False).collect()
        if results is None:
            results = res
        else:
            results = pl.concat([results, res], how="vertical")
            
    results.write_csv(f"chr{chrom}_height_gwas_results_no_hwe_no_norm_missing_removed.csv")
